...
if ($TAB eq 'GOOGLE-SAVE') {
	...
	$webdbref->{'google_tax_tables'} = $ZOOVY::cgiv->{'google_tax_tables'};
...
if ($TAB eq 'GOOGLE') {
	...
	$GTOOLS::TAG{'<!-- GOOGLE_TAX_TABLES_0 -->'} = ($webdbref->{'google_tax_tables'}==0)?'selected':'';
	$GTOOLS::TAG{'<!-- GOOGLE_TAX_TABLES_1 -->'} = ($webdbref->{'google_tax_tables'}==1)?'selected':'';
