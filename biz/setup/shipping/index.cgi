...
if (defined($ZOOVY::cgiv->{'MESSAGE'})) {
	$GTOOLS::TAG{"<!-- MESSAGE -->"} = "<br><table border='2' width='80%' align='center'><tr><td><center><font color='red' size='4' face='helvetica'><b>".$ZOOVY::cgiv->{'MESSAGE'}."</b></font></center></td></tr></table><br>";
	}
...
if ($VERB eq 'FEDEX-CONFIG') {
	...
	$GTOOLS::TAG{'<!-- METER_LINK -->'} = qq~
		<input class='button' type='button' onClick='document.location="index.cgi?VERB=FEDEX-PRT-REGISTER";' value='New Meter'>
		~;
...
if ($VERB eq 'BANNED') {
	...
	foreach my $line (split(/[\n\r]+/,$webdbref->{'banned'})) {
		...
		$c .= "<tr>";
		$c .= "<td><a href='index.cgi?ACTION=BANNED-NUKE&ID=$i'>[Del]</a></td>";
		$c .= "<td>$type</td>";
		$c .= "<td>$match</td>";
...
if ($VERB eq 'COUNTRIES') {
	...
	if ($FLAGS =~ /,SHIP,/) {
		...
		foreach my $isox (split(/,/,$webdbref->{'ship_blacklist'})) {
			...
			$c .= "<tr><td><a href='index.cgi?ACTION=FILTER-DELBLOCK&ISOX=$isox'>[Del]</a></td><td>$info->{'Z'} ($isox)</td></tr>";
			}
...
~,$webdbref->{'promotion_freeshipping_amount'});
	...
	foreach my $region ('US','CA','INT') {
		...
		foreach my $m (@{$methods}) {
			...
			$c .= qq~
<tr class="$r">
	<td valign=top><input type="button" class="button" value="Edit" onClick="
document.uniFrm.VERB.value='EDIT';
document.uniFrm.ID.value='$m->{'id'}';
document.uniFrm.submit();
"></td>
...
my @TABS = ();
push @TABS, { name=>'General', selected=>($VERB eq '')?1:0, link=>'index.cgi?ACTION=' };
push @TABS, { name=>'Countries', selected=>($VERB eq 'COUNTRIES')?1:0, link=>'index.cgi?ACTION=COUNTRIES' };
push @TABS, { name=>'Block-List', selected=>($VERB eq 'BANNED')?1:0, link=>'index.cgi?ACTION=BANNED' };
