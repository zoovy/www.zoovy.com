...
if ($ACTION eq 'COMPANYEDIT') {
	...
	<center>
		<input type="submit" name="BUTTON" value="Return to Product Editor" class="button2" onClick="document.location='/biz/product/edit.cgi?PID=".$SITE->pid()."';">
	</center>
		~;	
	...
	if (substr($SITE->pageid(),0,1) eq '*') {
		...
		push @TABS, { name=>'Live Preview', link=>'index.cgi?ACTION=LIVEPREVIEW&_SREF='.$SITEstr, color=>(($ACTION eq 'LIVEPREVIEW')?'orange':undef), };
		}
	push @TABS, { name=>'Meta Tags', link=>'index.cgi?ACTION=METAEDIT&_SREF='.$SITEstr, color=>(($ACTION eq 'METAEDIT')?'orange':undef), };
	push @BC, { name=>"Edit profile:".$SITE->profile()." format:".$SITE->format()." pageid:".$SITE->pageid()." layout:".$SITE->layout() };
	}	
...
if ($ACTION eq 'CHOOSER') {
	...
	$GTOOLS::TAG{'<!-- FLOW_CHOOSER -->'} = &TOXML::CHOOSER::buildChooser($SITE->username(),$SITE->format(),SUBTYPE=>$SITE->fs(),selected=>$SITE->layout(),'*LU'=>$LU);
	$SITEstr =  $SITE->siteserialize();
	$GTOOLS::TAG{'<!-- SREF -->'} = $SITEstr;
	}
	...
	if ($FORMAT eq '') {
		...
		if (defined $divsref) {
			...
			foreach my $divref (@{$divsref}) {
				...
				$BUF .= qq~<tr><td class="$class"><a href="index.cgi?ACTION=DIVSELECT&DIV=$divref->{'ID'}&_SREF=$SITEstr"
...
$BUF
		...
		# $BUF = &TOXML::PREVIEW::render_page($toxml,$SITE); 
		require TOXML::PREVIEW;
		require TOXML::EDIT;
		require SITE;

		($BUF) = $toxml->render('*SITE'=>$SITE,DIV=>$SITE->{'_DIV'});
#		$BUF = '<pre>'.Dumper({SREF=>$SITE,PREVIEW=>1,PROFILE=>$SITE->profile(),USERNAME=>$USERNAME,DIV=>$SITE->{'_DIV'}}).'</pre>';
#		$BUF .= Dumper($toxml);
		}
	else {
		$BUF = qq~
