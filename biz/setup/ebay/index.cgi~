...
if ($ACTION eq 'PROFILE-SHIPPING') {
	...
	if (1) {
		...
		foreach my $line (split(/[\n\r]+/,$NSREF->{'ebay:ship_domservices'})) {
			...
			my $delete = "<a href=\"/biz/setup/ebay/index.cgi?ACTION=PROFILE-SHIPPING-DOM-DELETE&CODE=$CODE&ID=$i\">DELETE</a>";
	...
	if (1) {
		...
		foreach my $line (split(/[\n\r]+/,$NSREF->{'ebay:ship_intservices'})) {
			...
			my $delete = "<a href=\"/biz/setup/ebay/index.cgi?ACTION=PROFILE-SHIPPING-INT-DELETE&CODE=$CODE&ID=$i\">DELETE</a>";
...
if ($ACTION eq 'PROFILE-CHOOSE_WIZARD') {
	...
	my $epnsref = &EBAY2::PROFILE::fetch($USERNAME,$PRT,$NS);	
	$GTOOLS::TAG{'<!-- WIZARD_CHOOSER -->'} = TOXML::CHOOSER::buildChooser($USERNAME,'WIZARD',NS=>$NS,selected=>$epnsref->{'ebay:template'});
...
#if ($ACTION eq 'PROFILE-REFRESH') {
#	my $PROFILE = '';
#	push @MSGS, "SUCCESS|BATCH:$BATCHID|+Created refresh batch job for profile $PROFILE";
...
#			}
		...
		$c .= "<td valign='top'>";
		$c .= "<a href=\"/biz/setup/ebay/index.cgi?ACTION=DEACTIVATE&ID=$act->{'ID'}\">[Deactivate]</a><br>";
		...
		# $c .= "<a target=\"_blank\" href=\"/biz/setup/ebay/redirhack.cgi?SignIn&m=$USERNAME&eb=$act->{'EBAY_USERNAME'}&sandbox=".($act->{'IS_SANDBOX'}?'on':'')."\"> [Refresh Authorization Token]</a><br>"; 
		$c .= "<a target=\"_blank\" href=\"/biz/setup/ebay/index.cgi?ACTION=REGISTER\"> [Refresh Authorization Token]</a><br>"; 
		$c .= "<br>";
		$c .= "</td>";
