...
if ($VERB eq '') { 
	...
	foreach $cat (keys %{$catalogref}) {
		...
		$c .= '[<a href="/biz/setup/search/index.cgi?ACTION=DELETE&CATALOG='.$hashref->{'CATALOG'}.'">DELETE</a>]<br>';
...
my @TABS = ();
push @TABS, { name=>"Catalogs", selected=>($VERB eq '')?1:0, link=>"/biz/setup/search/index.cgi?ACTION="  };
push @TABS, { name=>"Logs", selected=>($VERB eq 'LOGS')?1:0, link=>"/biz/setup/search/index.cgi?ACTION=LOGS"  };
push @TABS, { name=>"Catalog Debug", selected=>($VERB eq 'DEBUG')?1:0, link=>"/biz/setup/search/index.cgi?ACTION=DEBUG"  };
push @TABS, { name=>"Tuning", selected=>($VERB eq 'GLOBAL')?1:0, link=>"/biz/setup/search/index.cgi?ACTION=GLOBAL" };
