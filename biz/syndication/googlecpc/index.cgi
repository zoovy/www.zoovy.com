...
#	push @TABS, { name=>'Webdoc',selected=>($VERB eq 'WEBDOC')?1:0, link=>"?VERB=WEBDOC&DOC=51528&PROFILE=$PROFILE", };

	($so) = SYNDICATION->new($USERNAME,$PROFILE,'GCP');
...
if ($VERB eq '') {
	my ($so) = SYNDICATION->new($USERNAME,"#$PRT",'GCP');
	...
	foreach my $safe (sort $NC->paths($ROOTPATH)) {
		...
		$c .= "<tr>";
		$c .= "<td><input onClick=\"document.thisFrm.SAFE.value='$safe'; document.thisFrm.submit();\" type=\"button\" class=\"button\" value=\" Export \"></td>";
		$c .= "<td>$safe</td>";
		$c .= "<td>$pretty</td>";
		$c .= "</tr>\n";
		}
...
&GTOOLS::output(
   'title'=>'Google Adwords CPC Syndication',
   'file'=>$template_file,
   'header'=>'1',
   'help'=>'#51528',
   'tabs'=>\@TABS,
...
if ($VERB eq '') {
	...
	foreach my $ns (sort keys %{$profref}) {
		...
		$c .= " | ";
		$c .= "<a href=\"/biz/batch/index.cgi?VERB=ADD&EXEC=SYNDICATION&DST=GCP&PROFILE=$ns&GUID=$ts\">PUBLISH</a>";
		$c .= "</td>";
		$c .= "</tr>";
		my ($s) = SYNDICATION->new($USERNAME,$ns,'GCP');
...
if ($VERB eq 'EDIT') {
	...
	my ($so) = SYNDICATION->new($USERNAME,$PROFILE,'GCP');
	my ($DOMAIN,$ROOTPATH) = $so->syn_info();
	tie my %s, 'SYNDICATION', THIS=>$so;
