...
function PFGetResults(id,btn,txt) {
	...
	if (btn == 'showlist') {
		...
		jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/ShowList', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	else if (btn == 'showcat') {
		var postBody = 'm=PRODFINDER/ShowCat&id='+id+'&btn='+btn;
		jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/ShowCat', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	else if (btn == 'showmore') {
		var postBody = 'm=PRODFINDER/ShowMore&id='+id+'&btn='+btn;
		jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/ShowMore', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	else {
		// alert('id: '+id+' button:'+btn+' txt:'+txt);
		var postBody = 'm=PRODFINDER/Load&id='+id+'&btn='+btn+'&txt='+encodeURIComponent(txt);
		jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/Load', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
...
//	alert(jQuery('#'+selectorEscapeExpression(div)));
	...
	if (jQuery('#'+selectorEscapeExpression('PF/'+src)).val() == 1) {
		...
		jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/New', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
...
function showNavcatProducts(safe) { 
	...
	if (jQuery('#'+selectorEscapeExpression('show_products_navcat:'+safe)).val() == 1) {
		...
		jQuery.ajax('/biz/ajax/prototype.pl/NAVCAT/ShowProducts', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	}
	...
	for (x=0; x < totalRows; x++){
		if (tr.id == allTR[x].id) {
			...
			if (y>=0) {
				...
				jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/Flip', { dataType:"text",data: postBody,async: 1 } ) ;
				for (i=0;i<5;i++) {
...
function PFaddRemove(cb) {
	...
	if (cb.checked) {
		...
		jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/Insert', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		tr.addClass('rs');	// adds the selected class to the current tr
		}
	else {
		var postBody = 'm=PRODFINDER/Remove&pid='+pid+'&id='+id;
		jQuery.ajax('/biz/ajax/prototype.pl/PRODFINDER/Remove', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		tr.removeClass("rs");
		}
...
function addProducts(safe) {
	...
	jQuery.ajax('/biz/ajax/prototype.pl/NAVCAT/Products', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
	}
...
function addNew(safe,mode) {
	...
	if (mode==0) {
		...
		jQuery.ajax('/biz/ajax/prototype.pl/NAVCAT/Add', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	else {
		var thisDiv = jQuery('#'+selectorEscapeExpression('~'+safe));		
		thisDiv.html('');
...
function renameCat(safe,mode) {
	...
	if (mode==0) {
		...
		jQuery.ajax('/biz/ajax/prototype.pl/NAVCAT/Rename', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	}
...
function toggleCat(safe) {
	...
	if (icon.prop("src").indexOf('miniup.gif')>=0) { 
		...
		if (jQuery('#'+selectorEscapeExpression('_pid'))) { postBody = postBody + '&_pid='+jQuery('#\_pid').val(); }
		jQuery.ajax('/biz/ajax/prototype.pl/NAVCAT/Expand', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	else {
		thisDiv.html('Collapsing!');
		var postBody = 'm=NAVCAT/Collapse&safe='+safe;
		if (jQuery('#'+selectorEscapeExpression('_pid'))) { postBody = postBody + '&_pid='+jQuery('#\_pid').val(); }
		jQuery.ajax('/biz/ajax/prototype.pl/NAVCAT/Collapse', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	}
...
function deleteCat(safe,z) {
	...
	if (z==0) {
		...
		if (jQuery('#'+selectorEscapeExpression('_pid'))) { postBody = postBody + '&_pid='+jQuery('#\_pid').val(); }
		jQuery.ajax('/biz/ajax/prototype.pl/NAVCAT/Delete', { dataType:"text",data: postBody,async: 1,success: function(data, textStatus, jqXHR){ jHandleResponse(data);} } ) ;
		}
	else {
