...
if ($VERB eq 'PRODUCTS') {
	...
	if (scalar(@skus) > 1000) {
		...
		foreach my $sku (sort (@skus)) {
			...
			## SKU
			$c .= qq~<td valign=top nowrap><a href="#" onClick="navigateTo('#!products',{'pid':'$pid'});">$pid</a></td>~;
...
if ($VERB eq 'NON_CONF_ORDERS') {
	...
	while ( my $orderref = $sth->fetchrow_hashref() ) {
		...
		if ($orderref->{'FORMAT'} ne 'STOCK') {
			$c .= qq~<td><a href="#" onClick="jQuery('#ordersContent').empty(); return navigateTo('/biz/orders/view.cgi?ID=$orderref->{'OUR_ORDERID'}');">$orderref->{'OUR_ORDERID'}</a></td>~;
			}
		else {
...
if ($VERB eq 'ORDERS') {
	...
	foreach my $row (@ROWS) {
		...
		$c .= qq~<tr class="$class">~;
		$c .= qq~<td></td>~;
		$c .= qq~<td><a href="#" onClick="jQuery('#ordersContent').empty(); return navigateTo('/biz/orders/view.cgi?ID=$row->{'OUR_ORDERID'}');">$row->{'OUR_ORDERID'}</a></td>~;
		$c .= qq~<td>$row->{'SKU'}</td>~;
		$c .= qq~<td>$row->{'QTY'}</td>~;
		$c .= qq~<td></td>~;
	...
	foreach my $line (@ORDERS) {
		...
		## Store order
		$c .= qq~<td valign=top nowrap><a href="#" onClick="jQuery('#ordersContent').empty(); return navigateTo('/biz/orders/view.cgi?ID=$line->{'OUR_ORDERID'}');">$line->{'OUR_ORDERID'}</a> ~;
		if ($pool ne '') { $c .= "($pool)"; }
