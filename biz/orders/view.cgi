...
# $SITE::URL = SITE::URLS->new($USERNAME,secure=>1,prt=>$O2->prt(),sdomain=>$O2->in_get('our/sdomain'));
...
if (defined $ZOOVY::cgiv->{'EXIT'}) {
   ...
   $GTOOLS::TAG{"<!-- BILL_EMAIL -->"} = ($O2->in_get("bill/email"));	
	...
	if ($O2->in_get('bill/email') eq '') {
		...
		$GTOOLS::TAG{'<!-- CUSTOMER_EDIT -->'} = '';
		$GTOOLS::TAG{'<!-- CUSTOMER_EDIT -->'} = "<a href=\"javascript:openWindowNR('/biz/manage/customer/index.cgi?VERB=CREATE&EMAIL=".$O2->in_get('bill/email')."&ORDER=".$O2->oid()."&PRT=".$O2->prt()."\');\">Create Customer Account</a>";		
		}
	elsif ($CID != $O2->customerid()) {
		$GTOOLS::TAG{'<!-- CUSTOMER_EDIT -->'} = "<a href=\"javascript:openWindowNR('/biz/manage/customer/index.cgi?VERB=SEARCH-NOW&scope=CID&searchfor=$CID&link=".$O2->oid()."\');\">Link Customer Account</a>";		
		}
	elsif ($CID) {
...
if (!$CMD) { 
	($a) = &CART2::VIEW::as_html($O2,'INVOICE',{},$SITE);
	&build_buttons('VIEW',$USERNAME,$O2);
...
sub build_payment_status {
	...
	my $c = "";
	my $ps = substr($O2->payment_status(),0,1);
	...
	$c .= "PAYMENT METHOD: ".$O2->payment_method()."<br>";
  
	#if ($O2->payment_method() eq "CREDIT") {
   #  # $O2->in_get(("payment_cc_status")
   ...
   #  $c .= "PAYMENT_CC_STATUS: ".$O2->in_get(("payment_cc_status")."<br>";
#
#     # $O2->in_get(("payment_cc_results")
#     $c .= "PAYMENT_CC_RESULTS: ".$O2->in_get(("payment_cc_results")."<br>";
#
#     # $O2->in_get(("payment_cc_processor")
#     $c .= "PAYMENT_CC_PROCESSOR: ".$O2->in_get(("payment_cc_processor")."<br>";
#     }
	...
	$GTOOLS::TAG{'<!-- OPTIONSTR -->'} = qq~
		<table border='0' cellpadding='0' cellspacing='0'>
			...
			<tr>
				...
				$DELETE_ORDER_BUTTON
				<input type='button' class="button"  onClick=\"document.location='move.cgi?CMD=EMAIL&NS=$NS&order-$ID=on';\" value=\"  Send Email \">
				<input type='button' class="button"  onClick=\"document.location='move.cgi?CMD=EXPORT&order-$ID=on';\" value=\"  Export \">
