...
if (($VERB eq 'TASK-APPROVE') || ($VERB eq 'TASK-FEEDBACK') || ($VERB eq 'TICKET-ADDINTERACTION')) {	
	...
	($pstmt) = &DBINFO::insert($zdbh,'TICKET_FOLLOWUPS',{
		...
		CREATED_GMT=>$^T, CREATEDBY=>"$USERNAME*$LUSERNAME",
...
if ($VERB eq 'TICKET-VIEW') {
	...
	$GTOOLS::TAG{'<!-- TICKET -->'} = $ticketref->{'ID'};
	$GTOOLS::TAG{'<!-- UUID -->'} = $ticketref->{'UUID'};
...
<ul>
<li> If you received an Internal Server Error on a critical service - Acceptable Use, Free. Please let us know right now. 
<li> When the home page, category, or search is not loading (or corrupt), or cart is not letting items be added, or all customers in US cannot checkout - Acceptable Use, free (assuming it's not a blatant configuration issue)
<li> An employee ran a batch job which just destroyed the store (with no backup) - Acceptable Use, it will be billable, but getting back online is probably more important. 
...
if ($VERB eq 'TICKET-CLOSE') {
	...
	my $pstmt = "update TICKETS set DIRTY=0,CLOSED_GMT=unix_timestamp(now()),DISPOSITION='CLOSED' where ID=$qtID and MID=$MID";
	print STDERR $pstmt."\n";
	$zdbh->do($pstmt);

	my $NOTES = "CLOSED BY CUSTOMER\n\n".$ZOOVY::cgiv->{'notes'};

	my ($pstmt) = &DBINFO::insert($zdbh,'TICKET_FOLLOWUPS',{
		ID=>0, MID=>$MID, PARENT=>$TICKETID,
		CREATED_GMT=>$^T, CREATEDBY=>"$USERNAME*$LUSERNAME",
		COMMENT=>$NOTES,
		PUBLIC=>2
...
if ($VERB eq 'TICKET-CREATE') {
	...
	if (not defined $ZOOVY::cgiv->{'UUID'}) { $ZOOVY::cgiv->{'UUID'} = Data::GUID->new()->as_string(); }
	$GTOOLS::TAG{'<!-- UUID -->'} = $ZOOVY::cgiv->{'UUID'};
...
***********************************************~.$notes;
	...
	if ($TICKET>0) {
		...
		my $qtUUID = $zdbh->quote($UUID);
		$pstmt = "update TICKET_ATTACHED_FILES set TICKET=".$TICKET." where TICKET=0 and UUID=$qtUUID and MID=$MID /* $USERNAME */";
		print STDERR $pstmt."\n";
		$zdbh->do($pstmt);
		}		
...
sub build_ticket_attachments {
	my ($USERNAME,$TICKET,$TASK) = @_;
		...
		my $c = '<table></table>';
		$c = '';
		my $pstmt = "select ID,CREATED,UPLOAD_FILENAME,length(DATABLOB),md5(DATABLOB) from TICKET_ATTACHED_FILES where TICKET=".$zdbh->quote($TICKET)." and MID=$MID /* $USERNAME */ and PUBLIC>0 and TASK=$TASK";
		...
		while ( my ($fileid,$created,$uploaded,$size,$digest) = $sth->fetchrow()) {
			...
			my $link = "/biz/support/view.cgi/$uploaded?user=$USERNAME&ticket=$TICKET&fileid=$fileid&digest=$digest";
			$c .= qq~
			<tr>
				<td class='$class'> <a target="_blank" href="#" onClick="linkOffSite('$link'); return false;">$uploaded</a></td>
...
<div class="box">
<div class="box_header"> Legacy Projects &amp; Incidents</div>
<div class="box_body small">
$PROJECTS
</div></div>
...
&GTOOLS::output('*LU'=>$LU,'*LU'=>$LU,'*LU'=>$LU,header=>1,title=>"Support Area: $USERNAME($LUSERNAME) .. $VERB/$TICKETID",file=>$template_file,bc=>\@BC,tabs=>\@TABS,msgs=>\@MSGS);
