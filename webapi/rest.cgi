...
if ($EC==0) {
	...
	## set the minimum compatibility layer to 100
	if ($::XCOMPAT<108) { $EC = 1024; }
	## set the maximum compatibility value to 100
	elsif ($::XCOMPAT>=120) { $EC = 1025; } 
	}
...
if ($EC==0) {
	...
	if ($EC==0) {
		...
		if ((($CODE eq 'ZOME') || ($CODE eq 'ZOM') || ($CODE eq 'ZWM') || ($CODE eq '')) && ($SEAT eq '')) { $SEAT = 1; }
	...
	if ($EC==0) {
		...
		#	print STDERR "TOKEN: [$PASSWORD] ***\n";
			...
			if ($LUSER eq '') {
				...
				$pstmt = "select PASSWORD from ZUSER_LOGIN where MID=$MID and USERNAME=".$dbh->quote($USERNAME)." and LUSER=".$dbh->quote($LUSER);
				}
		...
		print STDERR "SERVER: $ENV{'SERVER_ADDR'}\n";
...
if ($::LEGACY) {
	...
	if ($EC!=0) {
		...
		if ($USERNAME eq 'toychest') { $::XERRORS++; }

		if ($::XERRORS) {
			use Data::Dumper;
			open F, ">/tmp/XERRORS-$USERNAME-$XREQUEST-$XCOMPRESS";
