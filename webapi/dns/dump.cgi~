...
foreach my $CLUSTER (@CLUSTERS) {
	...
	$sth->execute();
	while ( my ($USERNAME,$DOMAIN,$IPADDR) = $sth->fetchrow() ) {
		next if ($USERNAME eq '');
		$DOMAIN =~ s/^secure\.//i; 
		$SECURE_USERDOMAINS{ lc("$USERNAME|$DOMAIN") } = $IPADDR;
		}
	$sth->finish();
	...
	my $pstmt = "select MID,USERNAME,DOMAIN,STATUS,REG_TYPE,EMAIL_TYPE,EMAIL_CONFIG,WWW_HOST_TYPE,WWW_CONFIG,M_HOST_TYPE,M_CONFIG,APP_HOST_TYPE,APP_CONFIG,SSL_IPADDR,SSL_PROVISIONED_GMT,DKIM_PUBKEY,MODIFIED_GMT,NEWSLETTER_ENABLE from DOMAINS ";
	...
	while ( my $dref = $sth->fetchrow_hashref() ) {
		...
		if ($dref->{'SSL_IPADDR'} eq '') {
			my $USERDOMAIN = lc(sprintf("%s|%s",$dref->{'USERNAME'},$dref->{'DOMAIN'}));
			if (defined $SECURE_USERDOMAINS{$USERDOMAIN}) {
				$dref->{'SSL_IPADDR'} = $SECURE_USERDOMAINS{$USERDOMAIN};
				$dref->{'SSL_NOTE'} = "** SSL CERTIFICATE HAS NOT BEEN PROVISIONED AT THIS TIME **";
				}
			}
